...
- 지금까지 다룬 모든 정렬 알고리즘은 리스트의 요소를 '비교'하여 정렬함 (= 비교 기반 정렬)
- 킷값을 비교하지 않고도 효율적인 비교 기반 정렬들보다 이론적으로는 더 빨리 정렬할 수 있음
- 기수(radix)란 숫자의 자릿수를 말함
- 기수 정렬은 자릿수의 값에 따라 정렬함
- 기수 정렬은 다단계 정렬임
- 단계의 수는 데이터의 전체 자릿수와 같음
- 배분을 이용해 비교하지 않고 정렬함
- 값들을 적절히 배분해 놓았다가 순서대로 다시 모으는 것임
- 이를 위해 여러 개의 버킷을 사용하는데, 이것은 큐 구조임

**한 자리 자연수의 정렬**

- 한 자리의 자연수로만 이루어진 리스트 [8, 2, 7, 3, 5]를 정렬한다면,
- 십진수는 각 자리가 0에서 9까지의 값을 가지므로 먼저 10개의 버킷으르 준비해야 함
- 버킷들이 준비되면 입력 데이터를 값에 따라 버킷에 넣음
- 모든 숫자를 자신이 속한 버킷에 넣음(8은 8번 버킷에, 2는 2번 버킷에)
- 다음은 버킷에 있는 숫자를 순서대로 출력하는 것임
- 0번 버킷부터 시작하여 9번 버킷까지 순차적으로 버킷 안의 모든 숫자를 출력함
- 정렬된 숫자 리스트 [2, 3, 5, 7, 8]을 얻을 수 있음
- 이 과정에서 비교 연산은 전혀 사용되지 않음
- 레코드의 값에 따라 버킷에 넣고 빼는 동작만 되풀이했을 뿐임

**여러 자리 자연수의 정렬**

- 두 자리 자연수 리스트 [28, 93, 39, 81, 62, 72, 38, 26]을 정렬한다면,
- 가장 간단한 방법은 0에서 99번까지 번호가 매겨진 100개의 버킷을 사용하는 것임
- 다만 더 효과적으로 정렬하기 위해 1의 자리와 10의 자리를 따로따로 정렬할 수 있음
- 이렇게 하면 10개의 버킷만으로도 두 자리 정수를 정렬할 수 있음
- 낮은 자릿수를 먼저 처리하고 다음에 높은 자릿수를 처리해야 성공적으로 정렬됨
- 버킷은 큐로 구현됨

**기수 정렬의 특징**

- 기수 정렬은 비교 기반 정렬의 이론적인 하한선을 깰 수 있음
- 퀵 정렬과 같이 아무리 좋은 비교 기반 정렬 알고리즘도 nlog2n에 비례하는 시간 이상이 걸리는 것으로 알려져 있음
- 기수 정렬은 O(dn)의 복잡도를 가짐
- 이때 d는 단계의 수인데, 대부분은 크지 않은 값을 가짐
- 따라서 시간 복잡도 측면에서 퀵 정렬이나 병합 정렬보다 훨씬 우월함
- 다만, 정렬에 사용되는 킷값이 자연수로 표현되어야만 적용할 수 있음
- 만약 리스트의 요소가 실수이거나 한글, 한자라면 적용이 어려움
- 즉, 제한된 응용에만 사용할 수 있는 알고리즘임
- 더불어 버킷을 위한 추가적인 메모리가 필요함
- 많은 공간을 확보해 놓고 처리 시간을 줄이는 방법임
